...
async function testUnenrollmentDebug() {
  console.log('🧪 Debugging Unenrollment API\n');

  try {
    // 1. Create test enrollment first
    console.log('1. Setting up test enrollment...');
    const testUserId = `unenroll_test_${Date.now()}`;
    ...
    const { data: newStudent, error: studentError } = await supabase
      ...
      .insert({
        ...
        full_name: 'Unenroll Test Student',
        batch: '63',
        section: 'G'
      })
      .select()
    ...
    // 2. Test unenrollment API with detailed debugging
    console.log('\n2. Testing unenrollment API...');
    ...
    try {
      ...
      console.log('Unenrollment API response headers:', unenrollResult.headers);
